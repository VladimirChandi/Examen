...
const validarCedula = (cedula) => {
  ...
  return digitoVerificador === decimoDigitoCalculado;
};

app.post('/validar-cedula', async (req, res) => {
  const { cedula, nombres } = req.body;
  if (!cedula || !nombres) {
    return res.status(400).send({ error: 'La cédula y los nombres son requeridos' });
  }

  const esValida = validarCedula(cedula);
  if (!esValida) {
    return res.send({ cedula, nombres, esValida });
  }

  try {
    // Usar jsonplaceholder para simular la verificación de la cédula
    const response = await axios.get(`https://jsonplaceholder.typicode.com/posts/1`);
    const data = response.data;

    // Simulando una validación exitosa con el campo 'valida'
    res.send({ cedula, nombres, esValida, validacionExterna: true });
  } catch (error) {
